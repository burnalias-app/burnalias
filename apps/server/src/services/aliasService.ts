...
export class AliasService {
  ...
  async createAlias(input: z.infer<typeof createAliasSchema>): Promise<Alias> {
    ...
    if (!activeProvider.enabled) {
      log.warn({ providerId: activeProvider.id }, "Alias creation failed: provider is disabled");
      throw new Error("The active provider is disabled.");
    }
    ...
    const providerAlias = await provider.createAlias({
      ...
      domainName: activeProvider.type === "mock" ? activeProvider.config.aliasDomain : undefined
    });
  ...
  async setAliasStatus(id: string, nextStatus: Extract<AliasStatus, "active" | "disabled">): Promise<Alias> {
    ...
    this.auditLogRepository.create({
      aliasId: alias.id,
      eventType: nextStatus === "active" ? "alias.enabled" : "alias.disabled",
  ...
  async deleteAlias(id: string): Promise<void> {
    ...
    this.auditLogRepository.create({
      ...
      message: `Alias ${alias.email} deleted.`
    });
